...
const Login = () => {
  ...
  const handleJoinClick = async () => {
    ...
    const res = await fetch(`http://localhost:5000/check-user?phone=${userAPhone}`);
  ...
  const handleInvite = async () => {
    ...
    try {
      ...
      const res = await fetch(`http://localhost:5000/check-user?phone=${encodeURIComponent(userBPhone)}`);
      const data = await res.json();
